...
def heuristic_miscan(node):
    state = node.state
    return state.mis + state.can - (2 if state.boat else 0)

from buckets import greedy_search,a_star_search, print_solution

def main():
    goal = greedy_search(MisCanState(3, 3, True), goal_miscan_state, child_miscan_states,heuristic_miscan)
    print("Greedy Solution:")
    print_solution(goal)
    astar_goal = a_star_search(MisCanState(3, 3, True), goal_miscan_state, child_miscan_states, heuristic_miscan)
    print("A* Solution:")
    print_solution(astar_goal)
